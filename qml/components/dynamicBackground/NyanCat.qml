...
Shader
{
  ...
  iChannel0: Image { source: "qrc:/images/effects/tex14.png"; }
//  iChannel1: iChannel0;
  ...
  fragmentCode: "
    ...
    vec3 background(vec2 p)
    {
      ...
      visuv-=visuvm;
//      float vis = texture2D(iChannel1,vec2((visuv.x+1.0)*.5,0.0)).y*2.0-visuv.y;
...
//      float vis2 = texture2D(iChannel1,vec2(vis2p,0)).y;
...
//        color+=vec3(vis2,vis2,vis2)*0.5;
    ...
    {
      float vol = .0;
      // Noise code. Delays render A LOT
//      for(float x = 0.0; x<1.0; x+=1.0/256.0)
//      {
//        vol += texture2D(iChannel1,vec2(x,0.0)).y;
//      }
      vol*=1.0/256.0;
